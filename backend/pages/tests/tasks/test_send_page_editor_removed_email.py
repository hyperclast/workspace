...
from pages.tasks import send_page_editor_removed_email
from pages.tests.factories import PageEditorRemoveEventFactory
from users.tests.factories import UserFactory


class TestSendPageEditorRemovedEmailTask(TestCase):
    """Test send_page_editor_removed_email task function."""

    @patch.object(PageEditorRemoveEvent, "notify_user_by_email")
    def test_send_page_editor_removed_email_calls_notify_with_force_sync_true(self, mock_notify):
        """Test that task calls notify_user_by_email() with force_sync=True."""
        editor = UserFactory()
        event = PageEditorRemoveEventFactory(editor=editor, editor_email=editor.email)

        # Run task synchronously
        send_page_editor_removed_email(str(event.external_id))

        # Verify notify_user_by_email() was called with force_sync=True
        mock_notify.assert_called_once_with(force_sync=True)

    @patch.object(PageEditorRemoveEvent, "notify_user_by_email")
    def test_send_page_editor_removed_email_does_not_call_notify_when_event_not_found(self, mock_notify):
        """Test that task does not call notify when event doesn't exist."""
        fake_id = "non-existent-id"

        # Run task synchronously - should not raise exception
        send_page_editor_removed_email(fake_id)

        # Verify notify_user_by_email() was not called
        mock_notify.assert_not_called()

    def test_send_page_editor_removed_email_with_valid_event(self):
        """Test task with a valid event (integration test)."""
        editor = UserFactory()
        event = PageEditorRemoveEventFactory(editor=editor, editor_email=editor.email)

        # Patch the actual email sending to avoid real email operations
        with patch.object(PageEditorRemoveEvent, "notify_user_by_email") as mock_notify:
            send_page_editor_removed_email(str(event.external_id))
    ...
    def test_send_page_editor_removed_email_with_null_editor(self):
    ...
    def test_send_page_editor_removed_email_handles_exception_gracefully(self):
    ...
    def test_send_page_editor_removed_email_with_invalid_uuid_format(self):
        ...
        with patch.object(PageEditorRemoveEvent, "notify_user_by_email") as mock_notify:
            ...
            send_page_editor_removed_email(invalid_id)
