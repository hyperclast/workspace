...
class ReactionThrottle(UserRateThrottle):
    """
    Rate limits reaction toggles per authenticated user.

    Defaults to 60 requests per 60 seconds.
    """

    scope = "reaction_toggle"

    def __init__(self):
        pass

    def allow_request(self, request):
        requests = getattr(settings, "WS_REACTIONS_RATE_LIMIT_REQUESTS", 60)
        window = getattr(settings, "WS_REACTIONS_RATE_LIMIT_WINDOW_SECONDS", 60)
        self.rate = f"{requests}/{window}s"
        self.num_requests, self.duration = self.parse_rate(self.rate)
        return super().allow_request(request)
