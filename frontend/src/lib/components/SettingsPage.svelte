<script>
  ...
  const menuIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="1"></circle><circle cx="12" cy="5" r="1"></circle><circle cx="12" cy="19" r="1"></circle></svg>`;
  const addUserIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="8.5" cy="7" r="4"></circle><line x1="20" y1="8" x2="20" y2="14"></line><line x1="23" y1="11" x2="17" y2="11"></line></svg>`;
  ...
  onMount(() => {
    ...
    const getTabFromHash = () => {
      ...
      const validTabs = ["account", "org", "developer"];
  ...
  // Setup user avatar when user data loads
  $effect(() => {
    if (data.user?.email) {
      ...
      // Load gravatar for settings profile picture
  ...
  function switchTab(tab) {
    activeTab = tab;
    // Use replaceState to update URL without triggering popstate (which would remount the page)
  ...
  // Org handlers
  function toggleOrgMenu(e, orgId) {
    e.stopPropagation();
    openOrgMenuId = openOrgMenuId === orgId ? null : orgId;
  }
  ...
  async function handleRegenerateToken() {
    const confirmed = await confirm({
      ...
      description: "Your current token will be invalidated immediately. Any applications using the current token will stop working.",
          ...
          <div class="settings-profile-picture">
            ...
            <div class="profile-picture-info">
              ...
              <p class="profile-picture-hint">
                ...
                <a href="https://gravatar.com/profile" target="_blank" rel="noopener">Change it on Gravatar</a>
              </p>
            </div>
          </div>
          ...
          <div class="orgs-list">
            ...
            {:else}
              {#each data.orgs as org (org.external_id)}
                {@const canEdit = !org.domain && org.userRole === "admin"}
                <div class="org-card">
                  <div class="org-card-header">
                    ...
                    <div class="org-menu">
                      <button
                        class="org-menu-btn"
                        title="Options"
                        onclick={(e) => toggleOrgMenu(e, org.external_id)}
                      >
                        {@html menuIcon}
                      </button>
                      <div
                        class="org-menu-dropdown"
                        style:display={openOrgMenuId === org.external_id ? "block" : "none"}
                      >
                        <button
                          class="org-menu-item"
                        ...
                        {#if canEdit}
                          <button
                            class="org-menu-item"
                            onclick={(e) => handleRenameOrg(e, org)}
                          >
                            {@html editIcon}
                            Rename
                          </button>
      ...
      <div class="settings-tab-content" class:active={activeTab === "developer"}>
        <section class="settings-section">
          ...
          <div class="token-actions">
            <button class="btn-danger" onclick={handleRegenerateToken}>
              Regenerate Token
            </button>
