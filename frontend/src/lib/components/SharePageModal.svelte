<script>
  ...
  async function loadEditors() {
    ...
    try {
      const response = await csrfFetch(`${API_BASE_URL}/api/pages/${pageId}/editors/`);
      if (!response.ok) {
  ...
  async function confirmRemove(editor) {
    ...
    try {
      const response = await csrfFetch(
        `${API_BASE_URL}/api/pages/${pageId}/editors/${editor.external_id}/`,
        { method: 'DELETE' }
      );
...
<Modal bind:open title="Share This Page" onclose={handleClose}>
  {#snippet children()}
    <p class="modal-description">
      Invite collaborators to <strong>{pageTitle || 'this page'}</strong>
    </p>
