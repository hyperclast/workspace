...
const TEST_USER_EMAIL = USE_EXISTING_ACCOUNT
  ? EXISTING_EMAIL
  : `test-links-${Date.now()}@example.com`;
const TEST_USER_PASSWORD = USE_EXISTING_ACCOUNT ? EXISTING_PASSWORD : "TestPassword123!";

test.describe("Sidebar Links Navigation", () => {
  let page1Id = "";

  test("clicking a link in the Ref sidebar should navigate without breaking the app", async ({
    page,
  }) => {
    ...
    const refTab = page.locator(
      'button:has-text("Ref"), [data-tab="links"], .sidebar-tab:has-text("Ref")'
    );
    await refTab.click();
    await page.waitForTimeout(1000);
